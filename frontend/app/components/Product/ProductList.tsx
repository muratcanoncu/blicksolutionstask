					...
					{items.map((item, index) => (
						<tr key={item._id} className={`${item.bought ? "line-through text-orange-400" : ""} ${index === items.length - 1 ? '' : 'border-b border-orange-300'} group`} >
							<td className="p-3 pl-0 flex items-center">
								...
								<span onClick={() => toggleBought(item._id, item.bought)} className="cursor-pointer p-3 pr-8 break-all">{item.name}</span>
								<button className="invisible opacity-0 group-hover:visible group-focus-within:visible group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 rounded cursor-pointer py-2 px-3 bg-red-400 flex-none h-12" onClick={() => deleteItem(item._id)}>
